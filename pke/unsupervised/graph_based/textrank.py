...
class TextRank(LoadFile):
    ...
    def build_word_graph(self, window=10, pos=None):
        ...
        # the vertices are the words with allowed pos
        # the edges links words that co-occur in the window (x, x + size)
        # TODO why isn't the (x-size/2, x+size/2) window used ?
        # the edges weight is the frequency of the co-occurence
    ...
    def candidate_weighting(self, window=10, pos=None,
                            normalized=False, top_node=0.33):
        ...
        Args:
            ...
            normalized (False): normalize keyphrase score by their length,
                defaults to False.
            top_node (0.33): the ratio of top keywords to keep for
                post-processing, defaults to 0.33
        ...
        # t depends on the size of the document (Mihalcea and Tarau, 2004, 3.1)
        t = math.floor(len(self.graph.node) * top_node)
        w = sorted(w.items(), key=lambda x: x[1], reverse=True)
        w = w[:t]
        w = {k: v for k, v in w}

        # post-processing : creating multi-word keyword
